...
:root{--sticky:78px;}
/* Measured, not guessed: a 50px topbar plus the 66px size bar, and the same ~28px
   of clearance the index leaves above a heading it jumps to. It was 134 when the
   bar carried four lines under its track, which was already short of the chrome. */
body.sizepage{--sticky:144px;}
...
.sizebar-in{max-width:1120px;margin:0 auto;padding:9px 24px 11px;}
/* The label and the value share one row above the track and nothing sits below it,
   so the field's own vertical margins are collapsed here: this bar is fixed over a
   page of eight instruments, and every line in it is a line off every screenful of
   that page. What the bar draws *inside* its host is styled by size_bar.js, which
   is the file that creates it. */
.sizebar .sp-field label{margin-bottom:6px;}
.sizebar .sp-field output{margin-top:0;}
@media (max-width:720px){
  .sizebar-in{padding:7px 16px 9px;}
  /* Narrow, both bars grow a line: the topbar's nav wraps and the size bar's value
     drops under its label. */
  body.sizepage{--sticky:176px;}
}
